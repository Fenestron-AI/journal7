...
object AiDocumentsTable : Table("ai.documents") {
    ...
    val docCategory = varchar("doc_category", 64).nullable()
    val syncInterval = varchar("sync_interval", 16).nullable()
    val status = varchar("status", 32)
    val downloadState = text("download_state").nullable()
...
class PostgresAiRepository : AiRepository {
    ...
    private fun ResultRow.toDocument() = LegalDocument(
        ...
        docCategory = this[AiDocumentsTable.docCategory],
        syncInterval = this[AiDocumentsTable.syncInterval],
        status = DocumentStatus.valueOf(this[AiDocumentsTable.status]),
        downloadState = this[AiDocumentsTable.downloadState],
        processingState = this[AiDocumentsTable.processingState],
        priority = this[AiDocumentsTable.priority],
        pinned = this[AiDocumentsTable.pinned],
